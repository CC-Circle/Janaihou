...
public class turn_clock : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.tag == "Player")
        {
            this.transform.position = new Vector3(transform.position.x,transform.position.y+200,transform.position.z);

            StartCoroutine(DelayCoroutine(time_manager.repop_item_get, () =>
            {
                // n秒後にここの処理が実行される
                if (this.gameObject.transform.position.y > 100 && this.gameObject.GetComponent<turn_clock>().enable_revial == true)
                {
                    //Debug.Log("called");
